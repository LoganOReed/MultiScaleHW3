...
def avg_in_degree(g):
    """docstring for avg_in_degree"""
    print(g)
    avg_in_degree = sum(j for i,j in g.in_degree())
    print(f"with self-loops: {avg_in_degree / len(g.nodes)}")
    # Remove self loops
    g_temp = g
    for v in nx.selfloop_edges(g):
        g_temp.remove_edge(v[0], v[1])
    avg_in_degree = sum(j for i,j in g_temp.in_degree())
    print(f"without self-loops: {avg_in_degree / len(g.nodes)}")
...
def six():
    """6 Cell Lattice with periodic bdy conditions"""
    N = 6
    rule = {
            (1,1,1): 0,
            (1,1,0): 1,
    ...
    # print(nx.adjacency_matrix(g))
    adj = nx.to_numpy_array(g)
    plt.savefig(f'output/six/graph.png')
    # NOTE: Need to change 04b when updating N
    headers = [f'{x:06b}' for x in range(2**N)]
    with open('output/six/adj.tex', 'w') as f:
        print(tabulate(adj, headers, tablefmt="latex"), file=f)
    np.savetxt('output/six/adj.csv', adj, delimiter=',', header=','.join([f'{x:06b}' for x in range(2**N)]), fmt='%i')
    plt.close()
